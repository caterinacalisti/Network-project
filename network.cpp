...
size_t Network:: set_values(const std::vector<double>& val){
	...
	if(n<=size()){ //gère le cas où la tableau pris en paramètre a une taille plus pétite que values: alors on modifie que les premiers n valeurs de values
		for(size_t i(0); i<n; ++i){
			values[i] = val[i];}
			return n;}
	
	for(size_t i(0); i<size(); ++i){ // cas où la taille du tableau val est plus grande que celle de values: on modifie toutes les valeurs de values mais sa taille reste la mème
		values[i] = val[i];}
	return size();
}
